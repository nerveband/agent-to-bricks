...
| `@query` | Any Bricks element with query metadata |
...
**Two-step selection for `@section` and `@element`.** These types use a page-first flow: pick a page, then browse sections or elements within it. A back button lets you return to the page list. This keeps results focused instead of searching globally.

**Query and WooCommerce support for `@query`, `@product`, `@product-category`, and `@product-tag`.** Query mentions search the site's existing Bricks loops and query-enabled elements. Woo mentions read the live WooCommerce catalog metadata exposed by the plugin, so agents can reference actual products and taxonomies instead of guessing slugs.

**CSS framework support for `@color` and `@variable`.** Colors and variables are pulled from multiple sources: Bricks Builder's built-in palette, theme styles, and CSS files generated by frameworks like Automatic.css (ACSS) or Cwicly. If your site uses a CSS framework, its custom properties and colors will appear automatically.

You can also trigger autocomplete by typing the category directly, like `@page` followed by a space. Once you have typed at least two characters of a type name, the app matches it and jumps straight to search. Click anywhere outside the dropdown to dismiss it.
...
**Smart site context.** The site URL and name header is only sent with your first prompt in a session. Subsequent prompts skip the redundant header to keep things clean. If you need to re-send the connection context (for example, after reconnecting to a different site), click the **Reconnect** button in the prompt toolbar.
...
- **@mention references**, colored by type (blue for pages, purple for sections, green for elements, amber for classes, pink for colors, cyan for variables, orange for components, lime for media, gold for query/commerce objects). Hover over a mention pill to see a preview — color swatches for `@color`, image thumbnails for `@media`, and labels for `@query` and `@product`
...
- Quick-access mention chips (`@page`, `@section`, `@query`, `@product`, `@class`, etc.) below the input
...
**Combine mentions.** You can reference multiple objects in one prompt: "On `@page Shop`, restyle `@query Featured Products` to use `@product-category Hoodies` and match the `@color primary` token." Each mention adds its own resolved context.

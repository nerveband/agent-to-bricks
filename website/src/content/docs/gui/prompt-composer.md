...
- **@mention references**, colored by type (blue for pages, purple for sections, green for elements, amber for classes, pink for colors, cyan for variables, orange for components, lime for media). Hover over a mention pill to see a preview — color swatches for `@color`, image thumbnails for `@media`, and links for `@page`
